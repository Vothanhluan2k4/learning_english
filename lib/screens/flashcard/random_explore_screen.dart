...
import 'package:learning_english/models/explore_list.dart'; // Model ExploreList
import 'package:learning_english/models/explore_word.dart'; // Model ExploreWord
import 'package:learning_english/services/explore_service.dart'; // Service Explore
import 'package:flutter_tts/flutter_tts.dart';

class RandomExploreScreen extends StatefulWidget {
  final ExploreList list;

  const RandomExploreScreen({super.key, required this.list});

  @override
  State<RandomExploreScreen> createState() => _RandomExploreScreenState();
}

class _RandomExploreScreenState extends State<RandomExploreScreen> with SingleTickerProviderStateMixin {
  final ExploreService _exploreService = ExploreService(); // Sử dụng ExploreService

  List<ExploreWord> _allWords = [];
  ExploreWord? _currentWord; // Sử dụng ExploreWord
  ...
  // ===================================
  // 📚 LOGIC TẢI DỮ LIỆU TỪ EXPLORE SERVICE
  // ===================================
  Future<void> _loadAndShuffleWords() async {
    ...
    try {
      // Gọi getExploreWords từ ExploreService
      ...
      if (mounted) {
        print('Lỗi khi tải từ Explore: $e');
  ...
  Widget _buildNoticeBanner() {
    return Container(
      ...
      child: const Text(
        'Chế độ xem ngẫu nhiên (Khám phá). Nhấn vào thẻ để lật.',
        style: TextStyle(color: textPrimary, fontSize: 13),
      ),
    );
  }

  // Thay thế Word bằng ExploreWord
  Widget _buildFrontCard(ExploreWord word) {
    return Card(
      elevation: 6,
          ...
          children: [
            ...
            Align(
              ...
              child: Container(
                ...
                child: Text('Khám phá', style: TextStyle(color: primaryColor, fontSize: 13, fontWeight: FontWeight.bold)),
              ),
            ),
            const Spacer(),
            ...
            // Phiên âm
            // Sửa từ 'transcription' sang 'transcription' (đã đúng)
  ...
  // Thay thế Word bằng ExploreWord
  Widget _buildBackCard(ExploreWord word) {
    return Card(
      elevation: 6,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      color: Colors.grey.shade50,
            ...
            if (word.example?.isNotEmpty == true) ...[
              ...
              Container(
                ...
                child: Text(
                  word.example!,
                  textAlign: TextAlign.center,
                  style: TextStyle(fontSize: 18, fontStyle: FontStyle.italic, color: primaryColor),
                ),
              ),
            ],
            const Spacer(),
