...
class ModalverbsScreen extends StatefulWidget {
  ...
  @override
  State<ModalverbsScreen> createState() => _ModalverbsScreenState();
}

class _ModalverbsScreenState extends State<ModalverbsScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadLessons() async {
    try {
      ...
      final result = await _grammarService.getLessonsByTopicName('Modal Verbs');
      if (mounted) {
        setState(() {
          lessons = result;
          isLoading = false;
  ...
  Map<String, List<Lesson>> _groupLessons(List<Lesson> lessons) {
    ...
    for (var lesson in lessons) {
      ...
      if (title.contains('can') || title.contains('could')) {
        grouped['Can/Could']!.add(lesson);
      } else if (title.contains('may') || title.contains('might')) {
        grouped['May/Might']!.add(lesson);
      } else if (title.contains('must') || title.contains('should')) {
        grouped['Must/Should']!.add(lesson);
      } else {
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Icon(Icons.error_outline, color: Colors.teal[400], size: 60),
              ),
                      ...
                      child: InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => ModalverbsDetailScreen(
                                lessonId: lesson.id,
                              ),
                            ),
                          );
