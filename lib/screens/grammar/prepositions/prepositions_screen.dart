...
class _PrepositionsScreenState extends State<PrepositionsScreen> with TickerProviderStateMixin {
  ...
  Map<String, List<Lesson>> _groupLessons(List<Lesson> lessons) {
    ...
    for (var lesson in lessons) {
      ...
      if (title.contains('thời gian')) {
        grouped['Giới từ chỉ thời gian']!.add(lesson);
      } else if (title.contains('địa điểm')) {
        grouped['Giới từ chỉ địa điểm']!.add(lesson);
      } else if (title.contains('hướng')) {
        grouped['Giới từ chỉ hướng']!.add(lesson);
      } else {
  ...
  IconData _getGroupIcon(String groupName) {
    switch (groupName) {
      case 'Giới từ chỉ thời gian':
        return Icons.schedule;
      case 'Giới từ chỉ địa điểm':
        return Icons.location_on_outlined;
      case 'Giới từ chỉ hướng':
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Icon(Icons.error_outline, color: Colors.brown[400], size: 60),
              ),
                      ...
                      child: InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => PrepositionsDetailScreen(
                                lessonId: lesson.id,
                              ),
                            ),
                          );
