...
class _ReviewWrongQuestionsScreenState extends State<ReviewWrongQuestionsScreen> {
  final LearningService _service = LearningService(supabase: Supabase.instance.client);
  List<Map<String, dynamic>> wrongQuestions = [];
  bool isLoading = true;
  String? userId;
  bool _hasUpdatedData = false; // 🔥 NEW: Track if any data was updated
  ...
  Future<void> _markAsReviewed(String type, Map<String, dynamic> item) async {
    ...
    try {
      ...
      // 🔥 NEW: Mark that data has been updated
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope( // 🔥 NEW: Intercept back button
      onWillPop: () async {
        // Return true to indicate data needs refresh
        Navigator.pop(context, _hasUpdatedData);
